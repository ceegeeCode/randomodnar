...
'''
The code below -- all in a 'research state' -- was used for defining and training all convolutional-LSTMs reported in 
the following paper:
...
Notes:
    ...
    -- !OBS: ONLY THE WRAPPER allInOneWithDynSampling_ConvLSTMmodel HAS BEEN USED FOR THE PAPER!
    -- !OBS: OTHER "allInOne's" HAVE NOT BEEN USED FOR A LONG WHILE A WILL PROBABLY NEED WORK!
    -- !OBS: SIMILARLY ONLY THE MODEL makeConv1DLSTMmodel WAS USED FOR THE PAPER (and makeIdModel for sampling tests)!
    -- !OBS: ALL OTHER MODELS PROBABLY NEED TO BE REVISITED FOR WORKING PROPERLY! (MOST WERE USED FOR EARLIER EXPERIMENTS)
    
Changes:
dnaNet_LSTM: 26 mar '21; minor changes (now only 1 nrLSTMlayer param, nrLSTMLayer removed .. notice the capital L ...)  
...
In general: all functions called "allInOne"-something include/call all what is needed for training/validation for a 
particular model (see the OBS's above!). So allInOneWithDynSampling_ConvLSTMmodel, will train/test a LSTM model which uses a convolutional 
to take care of "word embedding"; the function calls the code for building the model, for compiling it and for 
dynamically sampling from the desired data. The parameters of the function allow to specify the model, the sampling 
and the training.

Jargon: the words 'test' and 'testing' here refers to what is elsewhere called validation. The proper testing
of the trained model, ie applying it to unseen data is taken care of in the dnaNet_stats module.
        
In the remaining part of this usage section, right below, you'll a find a lot of calls to 
allInOneWithDynSampling_ConvLSTMmodel (and more). First, you need to load this module (dnaNet_LSTM) 
and thereafter you can specify all values to run a call of allInOneWithDynSampling_ConvLSTMmodel. Such
a call sets up and trains the model as you have specified. In the definition of the function (allInOneWithDynSampling_ConvLSTMmodel)
you can of course see all its parameters, so what to specify. But it broadly takes: 
* specifying the DNA sequence you want to train on, so a file path; the code reads this in as a long string
a then one-hot encodes it; 
* parameters that define the training -- how much material should be used for the training and in what way; 
* the parameters used to define/determine the actual model; and finally
* some parameters for the optimization (we have used ADAM thoughout) and more.  

To understand properly what the various parameters do, you should read the code! But just a few words
on how the training is structured:
- It runs in a series of outerLoops (we have though only used nrOuterLoops = 1 loop, so not made use of this)
- each outer loop consists in a set of 'repeats' (nrOfRepeats in numbers) 
- each repeat consisting in a set of epochs (nrEpochs in numbers)
- each of which finally consists in stepsPerEpoch times updating the model parameters by means of averaging over
batchSize samples.

So: if you have 1 outer loop (nrOuterLoops = 1), 10 repeats (nrOfRepeats = 10), 100 epochs (nrEpochs = 100), 
10 steps each (stepsPerEpoch) of batchSize = 500, the model will have used 

1* 10 * 100 * 10 * 500  = 5000000

samples in the training (with 500000 in each of the each of the 1*10 repeats). The sampling is done dynamically
(and randomly). One can set how to split the training/testing by a dedicated parameter, trainTestSplitRatio (we
have used 0.8, meaning that train-test set is split 80%-20%). 

What is returned when calling allInOneWithDynSampling_ConvLSTMmodel?:
* files defining the trained model (per repeat)
* plots of accuracy/loss of the training and validation (per repeat)
* a text 'runDataFile' keeping a log of which parameter settings you used in this call

There are various options. One is to re-start training a particular model, starting
from some repeat-number (for which a trained model was stored in earlier training).
...
rootOutput =  r"/binf-isilon/kroghgrp/tkj375/various_python/DNA_proj/results_nets/ptPrecious/human/on_hg38/LSTM4S/trainTestSplit_80_20/"
    ...
    #    leftAndRight = concatenate([lstm_left_1, lstm_right_1], axis=-1)
        ...
        for j in range(nrOfParallelLSTMstacks):
            ...
            for i in range(nrLSTMLayers):
                ...
                print("LSTM layer: ", i)
                print("Left-hand shape after after i'th LSTM", lstm_left_i._keras_shape)
                print("Right-hand shape after i'th LSTM ",lstm_right_i._keras_shape)
...
#    inputs_right = Input(shape=(sequenceLength,letterShape))
    ...
    left_j = lstm_left 
    print("Shape of  LSTM output ", left_j._keras_shape)
    ...
    #                        Xleft[:, :(customFlankSize+overlap), :] = X[:, :(customFlankSize + overlap) , :]
        ...
        if testOfSamplingOnly_b == 1: #for testing the sampling
            ...
            if genSamples_b > 0.5:
                ...
                s += "Avoided data from these chromosomes: " +  str(avoidChromo)  + "\n" #moved her from 'if onlyOneRandomChromo_b == 1:' clause on 26 march 2021. Purpose: the training restricted to the 'odd numbered chromos'
