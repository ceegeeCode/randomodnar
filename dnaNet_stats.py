...
Secondary use: 
Runs aimed for revealing periodicity in base composition, AT- or GC-rich
...
modelName = modelName
saveAtDpi = 300
stats.scatterplotAnnoByChromo(inputDict = resultsDictChromo, chromosomeOrderList = chromosomeOrderList, modelName = modelName, nameChangeDict = {}, rootOutput = rootResults, saveAtDpi = saveAtDpi)
...
rootOutputTheModels = r"/isdata/kroghgrp/tkj375/various_python/DNA_proj/results_nets/ptPrecious/human/on_hg38/TheModels/4Models/"
...
chromosomeOrderList = ['hg38_chr19']
...
#Which input to use:
inputArrayType = 0 # ref base prob's; 0: pred returns
#plotOnlyNorm_b = 0 #default:1 
...
def predictOnChromosomes(rootGenome, 
    ...
    for chromoName in chromosomeOrderList:
        ...
        startAtSegment = 0
        if startAtSegmentDict.has_key(chromoName):
...
def predictAccrossGenomeInSegments(rootOutput, 
    ...
    if len(encodedGenomeData) != 4:
        
        print "You have run encodeGenome (dnaNet_dataGen module) without asking to have the genome sequece included in  output. Change and rerun."
...
#    if len(encodedGenomeData) != 4:
#        
#        print "You have run encodeGenome (dnaNet_dataGen module) without asking to have the genome sequence included in the output. Change and rerun."
#    
#    else:
#        
#        genomeSeq, repeatInfoSeq, exonicInfoSeq, genomeSeqString =  encodedGenomeData 
...
#    sampledPositionsBoolean = np.zeros(shape = lGenomeSeqCovered)
    ...
    for i in range(startAtSegment, nrSegments):        
        ...
        try:
            ...
            sampledPositionsSeg = map(int, sampledPositionsSeg) #an "unsafe" recasting but should be fine here ...
...
def calculateAggrAccOnChromos(rootOutput, chromosomeDict, dictionaryName = 'accuracyChromoByAnnoDictionary'):
    '''Compute the acc accross all chromos for each annotation type, as covered by the input
    ...
    for chromoName in chromosomeDict:
            ...
            try:
                ...
                if not(resultsDict.has_key(chromoName)):
                    resultsDict[chromoName] = {}
                    
                for anno in resultsDictByAnno:
                
                    if not(resultsDict[chromoName].has_key(anno)):
    ...
    for chromoName in resultsDict:
        ...
        for anno in resultsDict[chromoName]:
            ...
            if not(returnDict.has_key(anno)):
...
#        print dataDict
        ...
        for modelName in modelNameList:
            ...
            ax.bar(X + cnt*bar_width , dataDict[modelName], color = colors(cnt), width = bar_width, label = modelName)
...
def collectAccuracyChromosomesSeveralModels(rootOutput, 
    ...
    for model in modelFileList:
        ...
        for chromo in totAccDict:
            ...
            if not(returnDict.has_key(chromo)):
                returnDict[chromo] =  {}
                
            returnDict[chromo][modelName] = totAccDict[chromo]['all']
...
def scatterplotAnnoByChromo(inputDict, chromosomeOrderList = [], nameChangeDict = {}, modelName = '', rootOutput = '', saveAtDpi = 100):
    ...
    for anno in annoList: 
        ...
        if nameChangeDict.has_key(anno):
...
#                print inputArrayCopy[newStart:newEnd]
                 ...
                 if start <= newStart:
#                     print "inside 1"
                     tmp = inputArrayCopy[end:newEnd].copy()
                     inputArrayCopy[newStart:newEnd] = inputArrayCopy[start:end]
                     inputArrayCopy[start:newStart] = tmp
                 else:
#                     print "inside2"
                     tmp = inputArrayCopy[newStart:start].copy()
                     inputArrayCopy[newStart:newEnd] = inputArrayCopy[start:end]
                     inputArrayCopy[newEnd:end] = tmp
        except ValueError:
            print segmentId, idx, start,end, newStart, newEnd
...
#                    fig4.savefig(rootOutput_forATorGCbias + 'InvFourier_peak_shuffled_at' + str(nPeak)+ '_w' + str(fourierWindowLength) + '_step' + str(fourierStep) + '_' + genomeIdName + '_seg' + str(segStart) + '_' + str(cnt) + '_avgRevCompl' + str(averageRevComplementary_b) + '_win' + str(windowLength) + '_step' + str(stepSize) + '.pdf')
        ...
        if ratioQ > ratioQcutoff:
            ...
            if plots_b == 1:
                ...
                ax0.plot(Xs, YAbs, label = i)
                ax0.legend(loc = 'upper right', fontsize = 'small')
                
                #max coeff to-end plot:
                ax1_loc.plot(Xs, yMaxs, label = i)
                ax1_loc.legend(loc = 'upper right', fontsize = 'small')
                ax1.plot(Xs, yMaxs)
                ...
                ax2_loc.plot(Xs, yNorm, linestyle = lineStyle, alpha = ratioQ, label = str(i) + ' , Q: ' + ratioQlabel)
                ax2_loc.legend(loc = 'upper right', fontsize = 'small')
                ax2.plot(Xs, yNorm, linestyle = lineStyle, alpha = ratioQ)
    ...
    #            Ys = [fftAvgPred[n] for n in Xs]
                ...
                ax3.plot(XsEnd, YAbsEnd, label = i)
...
#    print intervalIdxs
    ...
    for i in range(nrSegments):
        ...
        while end < endPos and idx < lastIdx:
            ...
            #decide wheter to displace this interval (idx) or not:
...
#    raw_input("Sis, bare du ..")
    ...
    for chromoName in chromosomeOrderList:
        ...
        if not(chromosomeDict.has_key(chromoName)):
...
def plotRefPredVsRefPred(labelArray, predArray1 , predArray2, qualArrayShared, modelName1, modelName2, bins = 50, rootOutput = '', chromoId = '', annotationDict = {}, positionsArray = 0, nameChangeDict = {}, startAtPosition = 0, saveAtDpi = 100):
    ...
    fig = plt.figure()
    #plt.title('Prediction of ref. base acc to two models' )
...
#    plt.scatter(probArray1, probArray2, c = cmap(0), alpha = 0.5)
    ...
    for annoType in annotationDict:
        ...
        if nameChangeDict.has_key(annoType):
