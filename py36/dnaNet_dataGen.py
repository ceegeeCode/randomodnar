...
The version of this module used for the paper is kept as dnaNet_dataGen_v1. 
Changes this version: 
-- we include an option in fastReadGenome to also output the list of chromo's it has read data from. 
-- fct splitChromoStrings. See section "Split chromo strings in two chunks of alternating segements" below. 
Purpose is to split genome in two parts that can be used for training and testing, resp. (and keeping the same amount
of the chromo's in each part, unlike if splitting by chromo number).  
...
Special note on the functions splitGenomeInChromosomes and (fast)ReadGenome:
...
More detailed, difference means diff between results of using the new version and the _v1 version: 
...
#On the small part of the partition of GRCh38, part2:
import dnaNet_dataGen as dataGen
chromoNameBound = 100
rootGenome = '/isdata/kroghgrp/tkj375/data/DNA/human/GRCh38.p12/split/'
genomeFileName = 'part2.txt'
genomeShortName = 'hg38_part2'
rootOut = rootGenome
dataGen.splitGenomeInChromosomes(root = rootGenome, genomeFileName = genomeFileName, genomeShortName = genomeShortName, rootOut = rootOut, chromoNameBound = chromoNameBound)
...
######################################################
# Split chromo strings in two chunks of alternating segements
######################################################

#Human
rootOutput =r'/isdata/kroghgrp/tkj375/data/DNA/human/GRCh38.p12/'
chromoList =  ['hg38_chr22', 'hg38_chr21', 'hg38_chr20', 'hg38_chr19', 'hg38_chr18', 'hg38_chr17', 'hg38_chr16', 'hg38_chr15', 'hg38_chr14', 'hg38_chr13', 'hg38_chr12', 'hg38_chr11', 'hg38_chr10', 'hg38_chr9', 'hg38_chr8', 'hg38_chr7', 'hg38_chr6', 'hg38_chr5', 'hg38_chr4', 'hg38_chr3', 'hg38_chr2', 'hg38_chr1']
ext = '.txt'
chromoFileList = []
...
splitNrPositions = [400000, 100000 ]
dataGen.splitChromoStrings(chromoFileList = chromoFileList, splitNrPositions = splitNrPositions,lineLength = 80,  rootOutput = rootOutput, fileNamePart1 = fileNamePart1, fileNamePart2 = fileNamePart2)

#yeast
rootOutput =r"/isdata/kroghgrp/tkj375/data/DNA/yeast/R64/S288C_reference_genome_R64-1-1_20110203/"
...
######################################################
# Compute the base compositions in the genomes
######################################################

import dnaNet_dataGen as dataGen

#Human
fileGenome = r'/isdata/kroghgrp/wzx205/scratch/01.SNP/00.Data/GCF_000001405.38_GRCh38.p12_genomic_filter.fna'
chromoNameBound = 1000
rootOutput =r'/isdata/kroghgrp/tkj375/data/DNA/human/GRCh38.p12/'
...
Xall, X, Xrepeat, Xexonic, chromoList = dataGen.fastReadGenome(fileName = fileGenome, 
       exonicInfoBinaryFileName = '',
       ...
       randomChromo_b = 0, 
       avoidChromo = [],
       inclChromoListInOutput_b = 1)

L = len(X)
LnoWilds = L - X.count('W')
zeroSet = {'A', 'T', 'C', 'G'}
outDict = {}
tot = 0
for base in zeroSet:
    cntBase = X.count(base)
    fracBase = float(cntBase)/LnoWilds
    outDict[base] = fracBase
    tot += fracBase*fracBase
 
outDict['randomGuessAcc'] = tot

import cPickle as pickle

#dump the result
dumpFile = rootOutput + 'baseCompositionDict'
pickle.dump(outDict, open(dumpFile, "wb") )    
 
totDict['zebrafish'] = outDict   
'''
...
#Function written to split the genome seq corr to a set of chromo's into 
#and odd and an even part: the one will contain every other segment of a set length
# (eg 1Mb) of the chromosome strings and the other will contain the rest. 
#The purpose is to train a model in the one part and test it on the other, so
#as to check that indeed the model generalises well:
def splitChromoStrings(chromoFileList, rootOutput, splitNrPositions = [400000, 100000 ], lineLength = 80, fileNamePart1 =  r'/part1.txt',  fileNamePart2 =  r'/part2.txt'):
    '''
    Function written to split the genome seq corr to a set of chromo's into 
    and odd and an even part: the one will contain every other segment of a set length
    (eg 1Mb) of the chromosome strings and the other will contain the rest. 
    ...
    #open the two part-files for writing:
    part1_fileName = rootOutput + fileNamePart1
    part2_fileName = rootOutput + fileNamePart2
    #flush and reopen i append mode
    part1 = open(part1_fileName, 'w')
    part2 = open(part2_fileName, 'w')
    part1.close()
    part2.close()
        
    #line of wildcards to be used below:
    wLine ="W"  + "\n"
#    for k in range(80):
#        wLine = wLine + 'W'
    ...
    #While it's mayb e possible to open two files for writing, it appeared difficult; unwanted behaviour 
    #was the result, when both files were opened with file = open(...); when writing to file 'with open(...) as ...:' 
    #should be used acc to documentation. So:
    #We accomplish the task in two loops over the chromo-list: in the first we write the desired part to the part1 file
    #and in the second loop we handle the part2 writing:
    nrOfChromos = len(chromoFileList)
    doneTheseChromos = []
    with open(part1_fileName, 'a') as part1:
        for i in range(nrOfChromos):
            
    
            fileName = chromoFileList[i]
            ...
            while True:
                ...
                if lineCnt == 0: #just to try to get 1000000 lines in each chunk; first line is header
                    lines = handle.readlines(lineLength)  # read in first line ... though from the read out it seems its reading in some 102 lines, no matter what lineLength is ..!
#                    print lines[0]
#                    print lines[lineLength-1]
#                    print len(lines)
                    #print lines
#                    raw_input("S..... .. .....")

                    part1.writelines(lines) 

                    lineCnt = 1
                else:
                    if part1_b == 1 and part2_b == 0:
                        lines = handle.readlines(splitNrPositions[0])  # read in chunks of eg 400000 bytes, or about 400000/80  lines http://effbot.org/zone/readline-performance.htm 
#                        print len(lines)
#                        raw_input("part 1")
                        part1.writelines(lines)
        
                    elif part1_b == 0 and part2_b == 1:
                        lines = handle.readlines(splitNrPositions[1])  # read in chunks of eg 100000 bytes, or about 100000/80 lines http://effbot.org/zone/readline-performance.htm 
#                        print len(lines)
#                        raw_input("part2")
                ...
                #when no more lines in this chunk write a line of wildcard symbol to the file (this is only dome to be able to detect the aseembly points and eg to disqulify context containing it):
                if part1_b == 1:
                    part1.write(wLine)
                    part1_b = 0
                    part2_b = 1
#                    print("part1 2", part1_b, part2_b)
...
#                    print("part1 2", part1_b, part2_b)
            ...
            doneTheseChromos.append(fileName)
            handle.close()
    
    part1.close()
    
    
    print("doneTheseChromos for part 1: ", doneTheseChromos)
    print("Number of chromos in vs out: ", len(chromoFileList), len(doneTheseChromos))


### Now part2: the only diff to the loop aobve is that now we only write to the part2 file

    doneTheseChromos = []
    with open(part2_fileName, 'a') as part2:
        for i in range(nrOfChromos):
            ...
            while True:
                ...
                if lineCnt == 0: #just to try to get 1000000 lines in each chunk; first line is header
                    lines = handle.readlines(lineLength)  # read in first line
#                    print lines[0]
#                    raw_input("S..... .. .....")

                    part2.writelines(lines) 
                    lineCnt = 1
                else:
                    if part1_b == 1 and part2_b == 0:
                        lines = handle.readlines(splitNrPositions[0])  # read in chunks of eg 400000 bytes, or 400000/80  lines http://effbot.org/zone/readline-performance.htm 

                    elif part1_b == 0 and part2_b == 1:
                        lines = handle.readlines(splitNrPositions[1])  # read in chunks of eg 100000 bytes, or 100000/80 lines http://effbot.org/zone/readline-performance.htm 
                        #raw_input("...")
                        part2.writelines(lines)
...
#                    print("part1 2", part1_b, part2_b)
    ...
    print("doneTheseChromos for part 2: ", doneTheseChromos)
    print("Number of chromos in vs out: ", len(chromoFileList), len(doneTheseChromos))
...
#        effectiveEndAtPosition = effectiveStartAtPosition + lenXall -1
        ...
        if inclChromoListInOutput_b == 1:
            return Xall, X, Xrepeat, Xexonic, chromoList #, effectiveStartAtPosition, effectiveEndAtPosition
        else:
...
def encodeGenome(fileName, 
    ...
    if outputAsDict_b == 0:
        ...
        #Read in data from genome
        returnFromFastReadGenome = fastReadGenome(fileName = fileName, 
               exonicInfoBinaryFileName = exonicInfoBinaryFileName,
               chromoNameBound = chromoNameBound, 
               startAtPosition = startAtPosition,
               endAtPosition = endAtPosition,
...
#               outputGenomeString_b = outputGenomeString_b,
               randomChromo_b = randomChromo_b, 
               avoidChromo = avoidChromo,
               inclChromoListInOutput_b = inclChromoListInOutput_b)
        
        if inclChromoListInOutput_b == 1:
            Xall, X, Xrepeat, Xexonic, chromoList = returnFromFastReadGenome
        else:
            Xall, X, Xrepeat, Xexonic = returnFromFastReadGenome
            chromoList = ['placeHolder']
        ...
        if outputEncoded_b == 1:
            ...
            if outputEncodedOneHot_b == 1:
                ...
                if outputGenomeString_b == 1:
                    return Xenc, XencRepeat, XencExonic,  X, chromoList #, effectiveStartAtPosition, effectiveEndAtPosition

                else:
                    return Xenc, XencRepeat, XencExonic, 'genomeString_in_output_notCalled', chromoList #, effectiveStartAtPosition, effectiveEndAtPosition
                ...
                if outputGenomeString_b == 1:
                    ...
                    return Xenc, XencRepeat, XencExonic, 'noGenomeStringCalled', chromoList #, effectiveStartAtPosition, effectiveEndAtPosition
        
        
        else:# outputEncoded_b == 0:
    
            return X, Xrepeat, Xexonic, chromoList #, effectiveStartAtPosition, effectiveEndAtPosition
...
def getAllSamplesFromGenome(encodedGenomeData,
    ...
    if len(encodedGenomeData) == 5: #if encodeGenome was run with outputGenomeString_b =1:
        genomeSeq, repeatInfoSeq, exonicInfoSeq, genomeSeqAsString, chromoList =  encodedGenomeData
    else:
        genomeSeq, repeatInfoSeq, exonicInfoSeq, dummy_genomeSeq_string, chromoList =  encodedGenomeData
